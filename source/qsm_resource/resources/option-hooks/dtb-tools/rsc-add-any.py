...
class PnlRscTextureAddTool(gui_prx_widgets.PrxSessionWindow):
    ...
    def get_matches_by_patterns(cls, pattern_string, variants):
        patterns = list(map(lambda x: str(x).strip(), pattern_string.split(',')))
        ...
        if matches:
            ...
            with self.gui_progressing(maximum=len(matches), label='resource build process') as g_p:
                for i_file_variants in matches:
                    ...
                    i_file_type = ['file', 'directory'][bsc_storage.StgPath.get_is_directory(i_file_path)]
                    if i_file_type not in include_file_types:
                        continue
                    i_resource_n_p_o = bsc_core.BscStgParseOpt(name_pattern)
