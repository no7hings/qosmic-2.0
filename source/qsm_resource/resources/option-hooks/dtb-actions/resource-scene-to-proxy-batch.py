        ...
        def build_for_data(self):
            ...
            with w.gui_progressing(maximum=len(_file_paths)) as _g_p:
                for _i_scene_maya_file_path in _file_paths:
                    _g_p.do_update()
                    #
                    _i_fbx_file_opt = bsc_storage.StgFileOpt(_i_scene_maya_file_path)
                    #
                    _i_variants = scene_maya_directory_p_o.get_variants(_i_fbx_file_opt.get_directory_path())
                    #
                    _i_proxy_ass_directory_path = proxy_ass_directory_p_o.update_variants_to(**_i_variants).get_value()
                    _i_proxy_ass_file_path = '{}/{}.ass'.format(_i_proxy_ass_directory_path, _i_fbx_file_opt.get_name_base())
                    if _use_update_mode is True:
                        if _i_fbx_file_opt.get_timestamp_is_same_to(_i_proxy_ass_file_path) is True:
                            bsc_log.Log.trace_method_warning(
                                'scene to ass',
                                'non changed update for "{}"'.format(
                                    _i_proxy_ass_file_path
                                )
                            )
                            continue
                    #
                        ...
                        gnl_dcc_core.MayaProcess.generate_cmd_script(
                            bsc_core.ArgDictStringOpt(
                                option=dict(
                                    ...
                                    resource=_i_variants['resource'],
                                    version=_i_variants['version'],
                                    scene_maya_file=_i_scene_maya_file_path,
                                    with_proxy_ass=o.get('with_proxy_ass'),
                                    proxy_ass_file=_i_proxy_ass_file_path,
                                    use_update_mode=_use_update_mode,
                                    create_user_datas=True
                                )
                            ).to_string()
                        )
                    )
    ...
    def get_all_file_paths_fnc_():
        ...
        with w.gui_progressing(maximum=len(dtb_resources)) as _g_p:
            for _i_dtb_resource in dtb_resources:
                ...
                _i_storage_dtb_path = '{}/{}'.format(_i_dtb_version.path, 'scene_maya_directory')
                _i_directory_stg_path = dtb_opt.get_property(
                    _i_storage_dtb_path, 'location'
                )
                if _i_directory_stg_path:
                    _i_fbx_directory_opt = bsc_storage.StgDirectoryOpt(_i_directory_stg_path)
                    _i_fbx_file_paths = _i_fbx_directory_opt.get_file_paths(ext_includes=['.ma'])
