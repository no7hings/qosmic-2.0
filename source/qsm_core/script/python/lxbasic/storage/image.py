...
class ImgOiioMtd(object):
    ...
    def create_png_as_fill(cls, file_path, size, rgba):
        option = dict(
            size='{}x{}'.format(*size),
            color='{},{},{}'.format(*rgba),
            output=bsc_cor_raw.auto_encode(file_path)
        )
        cmd_args = [
            bsc_cor_execute.ExcBaseMtd.oiiotool(),
            '--create {size} 4',
...
class ImgOiioOpt(object):
    ...
    def convert_to(self, file_path_tgt):
        ...
        if file_opt_src.get_is_file() is True:
            ...
            cmd_args = [
                bsc_cor_execute.ExcBaseMtd.oiiotool(),
                '-i "{}"'.format(bsc_cor_raw.auto_encode(file_opt_src.get_path())),
                '--attrib:type=string DateTime "{}"'.format(time_mark),
                '--adjust-time ',
                '--threads 1',
                '-o "{}"'.format(bsc_cor_raw.auto_encode(file_opt_tgt.get_path())),
            ]
            bsc_cor_process.PrcBaseMtd.execute_with_result(
                ' '.join(cmd_args)
            )
    ...
    def generate_thumbnail(self, width=128, ext='.jpg'):
        ...
        if os.path.isfile(self._file_path):
            if os.path.exists(file_path_) is False:
                ...
                cmd_args = [
                    bsc_cor_execute.ExcBaseMtd.oiiotool(),
                    '-i "{}"'.format(bsc_cor_raw.auto_encode(self._file_path)),
                    '--resize {}x0'.format(width),
                    '-o "{}"'.format(file_path_)
                ]
                bsc_cor_process.PrcBaseMtd.execute_with_result(
