...
class DtbOptForResource(DtbBaseOpt):
    ...
    def generate(cls, category_group):
        ...
        cfg = bsc_resource.BscConfigure.get_yaml('database/library/resource-basic')
        if cfg is None:
            raise RuntimeError()

        cfg_extend = bsc_resource.BscConfigure.get_yaml('database/library/resource-{}'.format(category_group))
...
class DtbNodeOptForRscVersion(object):
    ...
    def get_hdri_file(self):
        ...
        if bsc_storage.StgPath.get_is_exists(path):
            return path
        return bsc_resource.BscResource.get('assets/library/hdri/srgb/StinsonBeach.png')
