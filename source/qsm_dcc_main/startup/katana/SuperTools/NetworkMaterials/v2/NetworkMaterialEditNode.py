...
import collections, contextlib, logging, operator, re, Enum as enum, Utils
from Katana import Configuration, FnAttribute, NodegraphAPI, NodeGraphView, Nodes3DAPI, PyXmlIO
...
from .NetworkMaterialEditUtil import LayoutNodesSearcher, LayoutNodesSorter, LayoutParameterExtractor, OpArgNodeReferenceSearcher, OpArgPaths
...
class NetworkMaterialEditNode(NetworkMaterialBaseNode):
    ...
    def __init__(self):
        ...
        genericOpNode.getParameter('inputBehavior').setValue('fail', NodegraphAPI.GetCurrentTime())
        ...
        incomingNode.getParameter('inputBehavior').setValue('fail', NodegraphAPI.GetCurrentTime())
    ...
    def isUserParameterEditingAllowed(self):
        """
        @rtype: C{bool}
        @return: C{False}. The NetworkMaterialEdit node type does not support
            user parameters.
        @since: Katana 4.5v1
        """
        return False
    ...
    def __populateFromInputMaterial(self, incomingMaterial, materialAttr):
        ...
        @type incomingMaterial: C{FnAttribute.GroupAttribute}
        @type materialAttr: C{FnAttribute.GroupAttribute}
        @rtype: C{bool}
    ...
    def __getEditedGeometryProducer(self):
        ...
        @rtype: L{FnGeolibProducers.GeometryProducer}
    ...
    def __getMaterialAttributesOfNode(self, node):
        ...
        @rtype: C{FnAttribute.GroupAttribute} or C{None}
    ...
    def __getGeometryProducer(self, node):
        ...
        @type node: C{NodegraphAPI.Node}
        @param node: Node to view attributes at.
        @rtype: L{FnGeolibProducers.GeometryProducer} or C{None}
    ...
    def __createNodeFromLayoutAttr(self, paramExtractor, nodeName, nodeLayoutAttr):
        ...
        if not node:
            ...
            if nodeType.getValue() == 'ShadingGroup':
                ...
                if isinstance(inputPorts, FnAttribute.DataAttribute) and isinstance(inputPortDisplayNames, FnAttribute.DataAttribute):
    ...
    def __setMaterialLocationCallbackOnNodes(self, upstreamMaterial):
        ...
        @type upstreamMaterial: C{FnAttribute.GroupAttribute}
    ...
    def __constructLayoutArgs(self, gb, genericOpArgs):
        ...
        if not incoming:
            return False
        incomingLayout = incoming.getChildByName('layout')
    ...
    def __updateLayoutEdits(self, gb, opArgs, incomingRoot, internalRoot):
        ...
        for pathTuple, incoming, internal in self.__layoutLeafs(tuple(), incomingRoot, internalRoot, opArgs):
            ...
            if self.__isConnectionLayoutAttr(argPath):
                ...
                incomingData = set(isinstance(incoming, FnAttribute.DataAttribute) and incoming.getData() or [])
                internalData = set(isinstance(internal, FnAttribute.DataAttribute) and internal.getData() or [])
                prevAdded = set(isinstance(prevEditedAttr, FnAttribute.DataAttribute) and prevEditedAttr.getData() or [])
                prevDeleted = set(isinstance(prevDeletedAttr, FnAttribute.DataAttribute) and prevDeletedAttr.getData() or [])
    ...
    def __onParameterFinalize(self, gb, genericOpArgs, node, param, **_kwargs):
        ...
        if param.getNumChildren():
            ...
            if param.getName() == 'enable':
                ...
                if isinstance(deletedParamsAttr, FnAttribute.DataAttribute):
    ...
    def __registerEventHandlers(self):
        ...
        * Start polling for batched events to process.
        ...
        if Configuration.get('KATANA_UI_MODE'):
            ...
            from UI4.Util.PluginManager import RegisterTimerCallback
            RegisterTimerCallback(self.__flushNodeGraphChangedEvents, interval=0.25, suppressCallbackDurationWarning=True)
    ...
    def __unregisterEventHandlers(self):
        ...
        * Stop polling for batched events to process.
        ...
        if Configuration.get('KATANA_UI_MODE'):
            from UI4.Util.PluginManager import UnregisterTimerCallback
            UnregisterTimerCallback(self.__flushNodeGraphChangedEvents)
    ...
    def _getIncomingSceneOpAndLocation(self, port, graphState, transaction):
        ...
        with NodegraphAPI.StackedLocalGraphState(graphState):
            genericOpNode = self._getNodeFromParam('__node_genericOp')
            sceneGraphLocation = self.getScenegraphLocation()
        return (
         genericOpNode._getOp(port, graphState, set(), transaction), sceneGraphLocation)
    ...
    def __attrToList(attr):
        ...
        """
        if isinstance(attr, FnAttribute.DataAttribute):
...
def _ExtractUserParamArray(newUserParamArray, userParamArrayAttr):
    ...
    @type userParamArrayAttr: C{FnAttribute.Attribute}
    @rtype: C{NodegraphAPI.Parameter}
    @param newUserParamArray: Parameter to update.
    @param userParamArrayAttr: Source attribute to query.
    @return: C{newUserParamArray)
    """
    if isinstance(userParamArrayAttr, FnAttribute.DataAttribute):
        attrList = list(userParamArrayAttr.getData())
        ...
        for i in range(nodeUserParamsAttr.getNumberOfChildren()):
            ...
            if userParamName and userParamType:
                ...
                if paramType == 'string':
                    ...
                    if isinstance(userParamValue, FnAttribute.DataAttribute):
                        newParam.setValue(list(userParamValue.getData()), 0)
...
def _ExtractShadingGroupPorts(outputPorts, inputPorts, node, nodeName):
    ...
    if outputPorts:
        for i in range(outputPorts.getNumberOfChildren()):
            ...
            if isinstance(tags, FnAttribute.DataAttribute):
                tagList = list(tags.getData())
