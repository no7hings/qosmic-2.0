...
class DagNode(_node.Node):
    ...
    @classmethod
    def to_name_strip_namespace(cls, path_or_name, namespace):
        return cls.to_name(path_or_name).lstrip(namespace+cls.NAMESPACESEP)
    ...
    @classmethod
    def find_descendants(cls, path, type_includes, no_intermediate=True):
        # type_includes can be string or list
        return cmds.ls(path, type=type_includes, dag=1, noIntermediate=no_intermediate, long=1) or []
