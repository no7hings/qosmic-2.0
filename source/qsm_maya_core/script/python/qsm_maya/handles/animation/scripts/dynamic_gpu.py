...
class DynamicGpuCacheOpt(qsm_mya_resource.AssetCacheOpt):
    CACHE_ROOT = qsm_mya_hdl_gnl_core.ResourceCacheNodes.DynamicGpuRoot
    CACHE_NAME = qsm_mya_hdl_gnl_core.ResourceCacheNodes.DynamicGpuName
    ...
    def generate_motion_args(self):
        if self._root is not None:
            ...
            directory_path = qsm_gnl_core.DccCache.generate_dynamic_gpu_directory(key=key)
    ...
    def generate_args(self, start_frame, end_frame, use_motion=False):
        ...
        directory_path = qsm_gnl_core.DccCache.generate_dynamic_gpu_local_directory()
