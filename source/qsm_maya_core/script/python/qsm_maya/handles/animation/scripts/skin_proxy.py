...
class SkinProxyOpt(qsm_mya_resource.AssetCacheOpt):
    ...
    def remove_resource_auto(self, head_geometries, rig_head_geometries, controls):
        ...
        paths_for_hide = self._resource.find_all_meshes()
        if paths_for_hide:
            # add skin proxy head
            if head_geometries:
    ...
    def generate_args(self):
        ...
        cache_file_path = qsm_gnl_core.DccCache.generate_skin_proxy_scene_file(
            file_path
        )
        data_file_path = qsm_gnl_core.DccCache.generate_skin_proxy_data_file(
            file_path
        )
...
class AdvSkinProxyGenerate(object):
    ...
    def create_resource_controls(self, location):
        if cmds.objExists(self.PROXY_CONTROL_PATH) is False:
            qsm_mya_core.SceneFile.import_file_ignore_error(bsc_resource.BscResource.get('rig/skin_proxy_control.ma'))
    ...
    def create_resource_geometries(self, location):
        if cmds.objExists(self.PROXY_GEOMETRY_GROUP_PATH) is False:
            if qsm_gnl_core.scheme_is_release():
                qsm_mya_core.SceneFile.import_file_ignore_error(
                    bsc_resource.BscResource.get('rig/skin_proxy_geometry_new.ma')
                )
            else:
                qsm_mya_core.SceneFile.import_file_ignore_error(
                    bsc_resource.BscResource.get('rig/skin_proxy_geometry.ma')
                )
    ...
    def create_cache(self, cache_file_path=None):
        with bsc_log.LogProcessContext.create(maximum=4) as l_p:
            ...
            if cmds.objExists(location) is False:
                if qsm_gnl_core.scheme_is_release():
                    qsm_mya_core.SceneFile.import_file_ignore_error(
                        bsc_resource.BscResource.get('rig/skin_proxy_new.ma')
                    )
                else:
                    qsm_mya_core.SceneFile.import_file_ignore_error(
                        bsc_resource.BscResource.get('rig/skin_proxy.ma')
                    )
            # step 2
