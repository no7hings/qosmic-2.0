...
class Stage(object):
    ...
    PTN_JSON_PATH = '{root}/lazy-resource/all/{key}/{node}/json/{node}.{tag}.json'

    PTN_MAYA_SCENE_PATH = '{root}/lazy-resource/all/{key}/{node}/maya/{node}.{tag}.ma'
    ...
    def upload_node_json(self, node_path, tag, data):
        ...
        json_path = self.PTN_JSON_PATH.format(**options)
    ...
    def generate_node_json_path(self, node_path, tag):
        ...
        return self.PTN_JSON_PATH.format(**options)
    ...
    def generate_node_maya_scene_path(self, node_path, tag):
        ...
        options['node'] = node_name
        options['tag'] = tag
        return self.PTN_MAYA_SCENE_PATH.format(**options)
