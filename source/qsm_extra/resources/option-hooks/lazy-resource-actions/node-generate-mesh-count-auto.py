...
import qsm_lazy_resource.resource_types.asset.scripts as lzy_rsc_etr_ast_scripts
    ...
    def _start_delay(window, task_window, scr_stage_name, scr_entities):
        ...
        with window.gui_progressing(maximum=len(scr_entities)) as g_p:
            for i_scr_entity in scr_entities:
                ...
                i_opt = lzy_rsc_etr_ast_scripts.AssetMeshCountGenerateOpt(
                    scr_stage_name, i_scr_entity_path
                )
                i_args = i_opt.generate_args()
                if i_args:
                    i_task_name, i_cmd_script, i_cache_path, i_image_path = i_args
        ...
        if process_args:
            ...
            for i_args in process_args:
                ...
                i_wgt.profile_accepted.connect(
                    functools.partial(i_opt.register_process_memory_usage_by_profile, 'mesh_count_generate')
                )
        else:
        ...
        if window is not None:
            ...
            if scr_entities:
                ...
                if task_window._language == 'chs':
                    task_window.set_window_title('生成网格计数')
                    task_window.set_tip(
                        '正在生成网格计数，请耐心等待；\n'
                        '如需要终止任务，请点击“关闭”。'
                    )
