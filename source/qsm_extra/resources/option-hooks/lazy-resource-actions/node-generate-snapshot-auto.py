    ...
    def _start_delay(window, task_window, scr_stage_name, scr_entities):
        ...
        if process_args:
            ...
            for i_args in process_args:
                ...
                i_wgt.profile_accepted.connect(i_opt.register_system_resource_usage_by_profile)
                i_wgt.profile_accepted.connect(
                    functools.partial(i_opt.register_process_memory_usage_by_profile, 'snapshot_generate')
                )
        else:
        ...
        if window is not None:
            ...
            if scr_entities:
                ...
                if task_window._language == 'chs':
                    task_window.set_window_title('生成快照')
                    task_window.set_tip(
                        '正在生成快照，请耐心等待；\n'
                        '如需要终止任务，请点击“关闭”。'
                    )
