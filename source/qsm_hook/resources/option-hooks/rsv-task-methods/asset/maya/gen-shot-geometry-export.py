    ...
    if any_scene_file_path is not None:
        if bsc_storage.StgFileMtd.get_is_exists(any_scene_file_path) is True:
            ...
            if rsv_scene_properties:
                ...
                #
                with_shot_geometry_usd = hook_option_opt.get('with_shot_geometry_usd') or False
                if with_shot_geometry_usd is True:
                    mya_rsv_objects.RsvDccShotGeometryHookOpt(
                        rsv_scene_properties,
                        hook_option_opt,
                    ).set_asset_shot_geometry_usd_export()
            else:
                raise RuntimeError()
        else:
            raise RuntimeError()
