...
class _QtSpcTaskThreadWorker(QtCore.QThread):
    started = qt_signal()
    running = qt_signal()
    finished = qt_signal()
    completed = qt_signal()
    ...
    def _do_progress_started(self, value):
    ...
    @staticmethod
    def generate(widget, item_model):
        t = _QtSpcTaskThreadWorker(widget, item_model=item_model)
        _QtSpcTaskThreadWorker.INDEX += 1
        t.set_entity(_QtSpcTaskThreadWorker.INDEX)
        return t
    ...
    def run(self):
        ...
        if self._kill_flag is False and self.parent()._thread_terminate_flag is False:
            with QtCore.QMutexLocker(self.parent()._thread_worker_mutex):
                self._stdout(
            ...
            try:
                ...
                with QtCore.QMutexLocker(self.parent()._thread_worker_mutex):
                    ct = time.time()-st
                    self._stdout(
                        'thread is finished at: {}, cost {}s\n'.format(self._entity, round(ct, 3))
                    )
