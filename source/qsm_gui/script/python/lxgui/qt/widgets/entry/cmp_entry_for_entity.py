...
#   widget
class QtCmpEntryForEntity(QtWidgets.QWidget):
    ...
    QT_ENTRY_BUBBLE_CLS = _wgt_bubble.QtPathBubble
    ...
    def __init__(self, *args, **kwargs):
        super(QtCmpEntryForEntity, self).__init__(*args, **kwargs)
        ...
        self._entry_bubble_widget = self.QT_ENTRY_BUBBLE_CLS()
        self._lot.addWidget(self._entry_bubble_widget)

        self.next_press_clicked = self._entry_bubble_widget.next_press_clicked

        self._entry_bubble_widget.component_press_dbl_clicked.connect(self._enter_component_at_)

        self._entry_bubble_widget.component_press_clicked.connect(self._choose_component_at_)
        self._entry_bubble_widget.component_wheeling.connect(self._switch_component_at_)
        ...
        if widget == self._entry_widget:
            if event.type() == QtCore.QEvent.KeyPress:
                ...
                # undo and redo
                    ...
                    self.key_down_and_alt_pressed.emit()
                    return True
    ...
    def _choose_component_at_(self, index):
        path = self._entry_bubble_widget._get_component_at_(index)
        if path.get_is_root() is False:
            pass
    ...
    def _do_next_wait_start_(self):
        self._next_name_texts = []
        self._entry_bubble_widget._start_next_wait_()
        # self._update_next_enable_()

    def _do_next_wait_end_(self):
        self._update_next_enable_()
        self._entry_bubble_widget._end_next_wait_()
    ...
    def _update_next_(self):
        self.next_path_accepted.emit(self._get_path_())
        self._update_next_enable_()

    def _update_next_enable_(self):
        self._entry_bubble_widget._set_next_enable_(bool(self._get_next_name_texts_()))

    def _set_root_text_(self, text):
        self._entry_bubble_widget._set_root_text_(text)
    ...
    def _get_path_text_(self):
        return self._entry_bubble_widget._get_path_text_()

    def _get_path_(self):
        return self._entry_bubble_widget._get_path_()
