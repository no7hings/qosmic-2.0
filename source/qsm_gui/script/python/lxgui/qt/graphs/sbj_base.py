...
class AbsQtNodeDef(AbsQtSbjBaseDef):
    ...
    def _do_press_move_start_(self, event):
        ...
        self._get_graph_()._do_node_move_or_resize_start_()
    ...
    def _do_press_move_end_(self, event):
        self._get_graph_()._do_node_move_or_resize_end_()
    ...
    # resize
    def _do_press_resize_start_(self, event):
        # point
    ...
    def _do_press_resize_end_(self, event):
        self._get_graph_()._do_node_move_or_resize_end_(self)

    def _resize_left_by_point_(self, d_point, offset_point=None):
        if offset_point is not None:
            pass
        else:
            # top left point
            ...
            x_o = x_0-x
            w, h = w_0+x_o, h_0
            self._resize_to_geometry_left_(x, y, w, h)
    
    def _resize_right_by_point_(self, d_point, start_size, offset_point=None):
        if offset_point is not None:
            pass
        else:
            ...
            x_o = x_0-x
            w, h = w_0+x_o, h_0
            self._resize_to_geometry_right_(w, h)

    def _resize_to_geometry_left_(self, x, y, w, h):
        self._update_basic_args_as_left_(x, y, w, h)

        self._refresh_widget_all_()

        self._update_attachments_()
    
    def _resize_to_geometry_right_(self, w, h):
        self._update_basic_args_as_right_(w, h)
