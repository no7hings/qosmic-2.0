...
class AbsQtNodeDef(AbsQtSbjBaseDef):
    ...
    # resize
    def _do_press_move_trim_(self, event):
        # point offset
        d_point = event.globalPos()-self._resize_start_point_0

        self._graph._do_node_press_move_trim_for_(
    ...
    def _trim_left_fnc_(self, d_point, offset_point=None):
        if offset_point is not None:
            ...
            x_o = x_0-x
            w, h = w_0+x_o, h_0
            self._trim_left_by_geometry_(x, y, w, h)

    def _trim_right_fnc_(self, d_point, start_size, offset_point=None):
        if offset_point is not None:
            pass
        else:
            ...
            x_o = x_0-x
            w, h = w_0+x_o, h_0
            self._trim_right_by_size_(w, h)

    def _trim_left_by_geometry_(self, x, y, w, h):
        self._update_basic_args_as_left_trim_(x, y, w, h)
