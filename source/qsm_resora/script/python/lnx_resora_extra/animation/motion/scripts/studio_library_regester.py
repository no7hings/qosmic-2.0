...
class STDotAnimRegister(object):
    def __init__(self, scr_stage, root_path, directory_path):
        if isinstance(scr_stage, lnx_scr_core.Stage) is False:
            raise RuntimeError()

        self._scr_stage = scr_stage
        self._root_path = root_path
        self._directory_path = bsc_core.ensure_unicode(directory_path)
    ...
    def execute(self):
        ...
        self._scr_stage.upload_node_preview_as_image_sequence(
        ...
        for i_index, i_key in enumerate(keys):
            if i_index > 0:
                ...
                if i_index == max_index:
                    self._scr_stage.create_type(i_path, gui_name=i_gui_name, gui_name_chs=i_gui_name_chs)
                else:
                    self._scr_stage.create_type_as_group(i_path, gui_name=i_gui_name, gui_name_chs=i_gui_name_chs)
...
class STDotAnimRegisterBatch(object):
    def __init__(self, root_path, directory_path):
        self._scr_stage = lnx_scr_core.Stage('motion_test')

        self._root_path = bsc_core.ensure_unicode(root_path)
        self._directory_path = bsc_core.ensure_unicode(directory_path)
