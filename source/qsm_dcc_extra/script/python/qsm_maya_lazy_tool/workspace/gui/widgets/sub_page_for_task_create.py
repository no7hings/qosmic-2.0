    ...
    def _on_apply(self):
        ...
        if page is not None:
            if qsm_mya_core.SceneFile.new_with_dialog() is True:
                task_parse = qsm_mya_task.TaskParse()
                ...
                task_scene_ptn_opt = task_parse.generate_asset_source_task_scene_pattern_opt_for(**kwargs)
                ...
                task_scene_ptn_opt_new = task_parse.generate_asset_source_task_scene_pattern_opt_for(**kwargs_new)

                scene_src_path = task_scene_ptn_opt_new.get_value()

                if bsc_storage.StgPath.get_is_file(scene_src_path) is False:
                    task_session = task_parse.generate_task_session_by_asset_source_scene_src(scene_src_path)
                    ...
                    thumbnail_ptn_opt = task_parse.generate_asset_source_task_scene_thumbnail_pattern_opt_for(**kwargs_new)
