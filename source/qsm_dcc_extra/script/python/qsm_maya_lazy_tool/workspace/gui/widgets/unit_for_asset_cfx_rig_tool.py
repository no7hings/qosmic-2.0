...
class _GuiResourceViewOpt(_abs_unit_for_task_tool.AbsGuiResourceViewForTaskTool):
    ...
    def do_gui_refresh_all(self, force=False):
        if self._page._task_tool_opt is not None:
            ...
            group_opt = qsm_mya_tsk_cfx_rig_core.AssetCfxRigGroupOpt()
...
class _PrxBasicToolset(_abs_unit_for_task_tool.AbsPrxToolsetForTaskTool):
    ...
    def on_add_to_bridge_geo_by_select(self):
        if self._unit._task_tool_opt is not None:
            self._unit._task_tool_opt.add_to_bridge_geo_by_select()

    def on_copy_as_bridge_geo_by_select(self):
        if self._unit._task_tool_opt is not None:
            self._unit._task_tool_opt.copy_as_bridge_geo_by_select(
                auto_blend=self._prx_options_node.get('bridge_geometry.auto_blend')
            )

    def on_copy_as_bridge_control_by_select(self):
        if self._unit._task_tool_opt is not None:
            self._unit._task_tool_opt.copy_as_bridge_control_by_select(
    ...
    def on_add_to_cloth_geo_by_select(self):
        if self._unit._task_tool_opt is not None:
            self._unit._task_tool_opt.add_to_cloth_geo_by_select()

    def on_copy_as_cloth_geo_by_select(self):
        if self._unit._task_tool_opt is not None:
            self._unit._task_tool_opt.copy_as_cloth_geo_by_select(
    ...
    def on_add_to_cloth_proxy_geo_by_select(self):
        if self._unit._task_tool_opt is not None:
            self._unit._task_tool_opt.add_to_cloth_proxy_geo_by_select()

    def on_copy_as_cloth_proxy_geo_by_select(self):
        if self._unit._task_tool_opt is not None:
            self._unit._task_tool_opt.copy_as_cloth_proxy_geo_by_select(
                auto_blend=self._prx_options_node.get('cloth.auto_blend')
            )
    ...
    def on_add_to_collider_geo_by_select(self):
        if self._unit._task_tool_opt is not None:
            self._unit._task_tool_opt.add_to_collider_geo_by_select()

    def on_copy_as_collider_geo_by_select(self):
        if self._unit._task_tool_opt is not None:
            self._unit._task_tool_opt.copy_as_collider_geo_by_select(
    ...
    def on_add_to_appendix_geo_by_select(self):
        if self._unit._task_tool_opt is not None:
            self._unit._task_tool_opt.add_to_appendix_geo_by_select()

    def on_copy_as_appendix_geo_by_select(self):
        if self._unit._task_tool_opt is not None:
            self._unit._task_tool_opt.copy_as_appendix_geo_by_select()

    # nucleus
    def on_create_ncloth_by_select(self):
        if self._unit._task_tool_opt is not None:
            self._unit._task_tool_opt.create_ncloth_by_select()

    def on_create_nrigid_by_select(self):
        if self._unit._task_tool_opt is not None:
            self._unit._task_tool_opt.create_nrigid_by_select()
        ...
        if self._unit._task_tool_opt is not None:
            result = self._window.exec_message_dialog(
                self._window.choice_message(
                    self._window._configure.get('build.{}.messages.auto_collection'.format(self._page.GUI_KEY))
                ),
                status='warning'
            )
            if result is True:
                self._unit._task_tool_opt.auto_collection()

    def on_auto_name(self):
        if self._unit._task_tool_opt is not None:
            result = self._window.exec_message_dialog(
                self._window.choice_message(
                    self._window._configure.get('build.{}.messages.auto_name'.format(self._page.GUI_KEY))
                ),
                status='warning'
            )
            if result is True:
                self._unit._task_tool_opt.auto_name()

    def on_auto_connection(self):
        if self._unit._task_tool_opt is not None:
            result = self._window.exec_message_dialog(
                self._window.choice_message(
                    self._window._configure.get('build.{}.messages.auto_connection'.format(self._page.GUI_KEY))
                ),
                status='warning'
            )
            if result is True:
                self._unit._task_tool_opt.auto_connection()

    def on_rest_rig_controls_transformation(self):
        if self._unit._task_tool_opt is not None:
            self._unit._task_tool_opt.rest_rig_controls_transformation()
