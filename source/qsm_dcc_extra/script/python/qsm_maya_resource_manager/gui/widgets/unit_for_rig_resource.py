...
class _GuiRigTagOpt(
    ...
    def __init__(self, window, unit, session, prx_tree_view):
        super(_GuiRigTagOpt, self).__init__(window, unit, session)
...
class _GuiRigOpt(
    ...
    def do_gui_refresh_tools(self):
        ...
        if self._window.window_is_active():
            isolate_tool = self._tool_dict['isolate-select-rig']
            isolate_select_opt = qsm_mya_core.ViewPanelIsolateSelectOpt(panel_current)
    ...
    def do_dcc_select_rigs(self):
        if self._prx_tree_view.has_focus() is True:
            ...
            paths = []
            if rigs:
                scheme = self._unit._rig_utility_options_node.get('selection_scheme')
    ...
    def __init__(self, window, unit, session, prx_tree_view):
        super(_GuiRigOpt, self).__init__(window, unit, session)
    ...
    def gui_add_rig(self, rig):
        def build_fnc_():
            ...
            if _reference_node.is_loaded():
                ...
                self._unit._gui_rig_tag_opt.gui_register_tag_by_path(
...
class _GuiRigReferenceOpt(
    ...
    def __init__(self, window, unit, session, prx_input_for_asset):
        super(_GuiRigReferenceOpt, self).__init__(window, unit, session)
    ...
    def gui_setup_unit(self):
        ...
        self._prx_rig_tab_group.add_widget(
            ...
            name=gui_core.GuiUtil.choice_name(
                self._window._language, self._session.configure.get('build.tag-groups.utility')
            ),
        ...
        self._rig_utility_options_node.set(
            'skin_proxy.remove_skin_proxy', self._gui_rig_opt.do_dcc_remove_skin_proxy
        )
        ...
        self._rig_utility_options_node.set(
            'dynamic_gpu.remove_dynamic_gpu', self._gui_rig_opt.do_dcc_remove_dynamic_gpu
        )
        ...
        self._prx_rig_tab_group.add_widget(
            ...
            name=gui_core.GuiUtil.choice_name(
                self._window._language, self._session.configure.get('build.tag-groups.extend')
            ),
            tool_tip=gui_core.GuiUtil.choice_tool_tip(
                self._window._language, self._session.configure.get('build.tag-groups.extend')
            )
        ...
        self._window.connect_window_close_to(self._destroy_rig_selection_script_job)
    ...
    def do_dcc_load_skin_proxies_by_selection(self):
        if self._load_skin_proxy_button.get_is_started() is False:
            ...
            if namespaces:
                ...
                if create_cmds:
                    mtd = _GuiProcessOpt(self._window, self)
