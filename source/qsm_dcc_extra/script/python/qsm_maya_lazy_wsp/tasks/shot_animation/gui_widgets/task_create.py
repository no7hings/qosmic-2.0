...
# cfx cloth
class PrxSubpageForShotAnimationCreate(_sub_page_for_task_create.AbsPrxSubpageForTaskCreate):
    TASK_CREATE_OPT_CLS = _task_create_opt.MayaShotAnimationCreateOpt

    GUI_KEY = TASK_CREATE_OPT_CLS.TASK

    def __init__(self, window, session, sub_window, *args, **kwargs):
        super(PrxSubpageForShotAnimationCreate, self).__init__(window, session, sub_window, *args, **kwargs)
    ...
    def do_gui_refresh_all(self):
        ...
        animation_preview_pth_opt = task_parse.generate_pattern_opt_for(
            'shot-disorder-layout-preview-file', **kwargs_new
        )
