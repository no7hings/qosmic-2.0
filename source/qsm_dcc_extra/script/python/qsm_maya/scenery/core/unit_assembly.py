...
class UnitAssemblyOpt(object):
    IMPORT_ROOT = '|__UNIT_ASSEMBLY_IMPORT__'
    ...
    def set_lod(self, lod):
        ...
        if active.startswith(_base.Assembly.Keys.GPU):
            if lod == 0:
                self.set_active(_base.Assembly.Keys.GPU)
            else:
                self.set_active(_base.Assembly.Keys.GPU_LOD.format(lod))
        elif active.startswith(_base.Assembly.Keys.Mesh):
            if lod == 0:
                self.set_active(_base.Assembly.Keys.Mesh)
            else:
                self.set_active(_base.Assembly.Keys.Mesh_LOD.format(lod))
    ...
    def to_mapper(self, paths):
        ...
        for i_path in paths:
            ...
            if i_result is not None:
                i_active = UnitAssemblyOpt(i_result).get_active()
                dict_.setdefault(
                    i_active, set()
                ).add(i_result)
        return dict_
