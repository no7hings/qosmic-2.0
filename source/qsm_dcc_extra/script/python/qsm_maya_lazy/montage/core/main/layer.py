...
class MtgLayerBase(object):
    ...
    def create_as_layer(cls, namespace):
        ...
        if qsm_mya_core.Node.is_exists(location) is False:
            clip_location = _bas_graph.MotionTimeGraph(namespace).create_all()
            location = _bas_graph.MotionLayerGraph(
...
class MtgLayer(AbsMtgLayer):
    ...
    def create_for(cls, namespace):
        is_create, location = MtgLayerBase.create_as_layer(
            namespace
        )
...
class MtgMasterLayer(AbsMtgLayer):
    ...
    @qsm_mya_core.Undo.execute
    def create_for(cls, namespace):
        is_create, location = MtgLayerBase.create_as_master_layer(namespace)
        return cls(location)
    ...
    def export_motion_to(self, file_path):
        ...
        resource = _adv_resource.AdvResource(namespace)
    ...
    def connect_to_adv(self, resource):
        qsm_mya_core.NodeAttribute.set_as_message(
            self._location, 'qsm_resource', resource.get_root()
        )
        resource.fit_master_layer_scale(self)
        resource.fit_layer_sketches(self)
        resource.connect_from_master_layer(self)

    def connect_to_mixamo(self, resource):
        qsm_mya_core.NodeAttribute.set_as_message(
            self._location, 'qsm_resource', resource.get_root()
        )
        resource.fit_master_layer_scale(self)
    ...
    def append_layer(self, file_path, pre_cycle=1, post_cycle=1, pre_blend=2, post_blend=2):
        ...
        layers_opt = MtgLayersOpt(self.get_all_layers())
        ...
        motion_layer_opt = MtgLayer.create_for(layer_namespace)
